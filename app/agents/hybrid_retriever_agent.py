...
from app.graph_db.graph_retriever import search_graph_products
...
class HybridRetrieverAgent:
    def run(self, current_requirements: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if current_requirements.get("payload"):
            query_parts.append(f"tải trọng {current_requirements['payload']} kg")
        if current_requirements.get("fuel_type"):
            query_parts.append(f"nhiên liệu {current_requirements['fuel_type']}")
        if current_requirements.get("budget"):
            query_parts.append(f"giá dưới {current_requirements['budget']} triệu")
            
        semantic_query = " ".join(query_parts)
        if not semantic_query:
            semantic_query = "Xe tải thương mại xe van chở hàng"
            
        logger.info(f"HybridRetriever running vector query: '{semantic_query}'")
        
        # 2. Execute parallel DB retrievals
        vector_results = search_vector_products(semantic_query, top_k=vector_limit)
        graph_results = search_graph_products(current_requirements, top_k=graph_limit)
        
        # 3. Merge results using Reciprocal Rank Fusion (RRF)
        merged_products = self._rrf_merge(vector_results, graph_results, reqs=current_requirements, limit=top_k)
        ...
        if not merged_products:
            logger.info("No exact matches found. Searching for close alternative commercial vehicles...")
            ...
            merged_products = self._rrf_merge(vector_results, graph_results, reqs=relaxed_reqs, limit=top_k)
            for p in merged_products:
                p["is_alternative"] = True
        
        logger.info(f"HybridRetriever merged {len(merged_products)} products for recommendation.")
        return merged_products

    def _rrf_merge(self, vector_results: list, graph_results: list, reqs: dict, limit: int = 3) -> List[Dict[str, Any]]:
        """
        ...
        def passes_constraints(p: dict) -> bool:
            ...
            req_payload = reqs.get("payload")
            if req_payload:
                if payload < float(req_payload):
                    return False
        ...
        # Process graph rankings with constraints check
        for rank, p in enumerate(graph_results):
            if not passes_constraints(p):
                continue
            pid = p["product_id"]
            product_pool[pid] = p
